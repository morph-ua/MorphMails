...
func (cq *ConnectorQuery) QueryReceivers() *ReceiverQuery {
	query := (&ReceiverClient{config: cq.config}).Query()
...
func (cq *ConnectorQuery) Clone() *ConnectorQuery {
	...
	return &ConnectorQuery{
		...
		withReceivers: cq.withReceivers.Clone(),
...
func (cq *ConnectorQuery) WithReceivers(opts ...func(*ReceiverQuery)) *ConnectorQuery {
	query := (&ReceiverClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withReceivers = query
	return cq
}
...
func (cq *ConnectorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Connector, error) {
	...
	if query := cq.withReceivers; query != nil {
		if err := cq.loadReceivers(ctx, query, nodes,
			func(n *Connector) { n.Edges.Receivers = []*Receiver{} },
			func(n *Connector, e *Receiver) { n.Edges.Receivers = append(n.Edges.Receivers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ConnectorQuery) loadReceivers(ctx context.Context, query *ReceiverQuery, nodes []*Connector, init func(*Connector), assign func(*Connector, *Receiver)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Connector)
	for i := range nodes {
	...
	query.withFKs = true
	query.Where(predicate.Receiver(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(connector.ReceiversColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.connector_receivers
		if fk == nil {
			return fmt.Errorf(`foreign-key "connector_receivers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "connector_receivers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
